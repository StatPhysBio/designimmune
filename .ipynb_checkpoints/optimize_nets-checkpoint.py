...
def run(
    ...
    # Find psis to run
    if "sparse-reg" in comment: # or "limit-E" in comment: 
        # or "larger-psi" in comment: # or "compete_d_E" in comment: # or "vary_b_I" in comment: # or "simple_d_E" in comment:
    ...
    psi_list = Parallel(n_jobs=num_cpu + add_cpu, batch_size=batch_size)(delayed(lin_stoch_sim)(
        d_I = param[2], K_I = param[1], b_I = param[0], K_H = param[3], N_0 = param[4], I_0 = param[5], K_S = param[6],
        d_IE = param[7], d_H = param[8], max_Na = param[9], max_expand = param[10],
