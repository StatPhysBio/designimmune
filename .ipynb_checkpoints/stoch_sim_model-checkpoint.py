...
d_A = 0.3
b_Ain = 1
delta = 10 # relative avidity of APCs for T cells compared to infected cells
K_Ain = K_IE/delta
...
max_Na = 4
max_expand = 20_000
t_act, t_bind, t_Na_div, t_E_div, t_cM_div, t_EeM_diff, t_E_out, t_E_die, t_E_cyt, t_NaE_diff = 1/2, 3/4, 1/4, 1/3, 1/2, 8, 1.0, 3.0, 1.0, 1/2
n_act, n_bind, n_Na_div, n_E_div, n_cM_div, n_EeM_diff, n_E_out, n_E_die, n_E_cyt, n_NaE_diff = 1, 1, 1, 1, 1, 1, 1, 8, 1, 1
    ...
    for k in np.arange(0, infection_count):
        ...
        for i in np.arange(1, int(steps) + 1):
            ...
            for j in np.arange(N_0_var):
                ...
                b_EineM_diff[j] = (1 - (1 - p_EineM[j])**(1/trans_steps[7]))/(1 - p_EineM[j])**(1/trans_steps[7])*np.minimum(trans_steps[7]/char_times[7], 1/(dt*trans_steps[7])) if Ein_m[i,j] > 0 else 0.0
                b_EouteM_diff[j] = (1 - (1 - p_EouteM[j])**(1/trans_steps[7]))/(1 - p_EouteM[j])**(1/trans_steps[7])*np.minimum(trans_steps[7]/char_times[7], 1/(dt*trans_steps[7])) if Eout_m[i,j] > 0 else 0.0
...
#     H = init[4]
#     N = init[5:N_0 + 5]
#     Na = init[N_0 + 5: 2*N_0 + 5]
#     E = init[2*N_0 + 5: 3*N_0 + 5]
#     cM = init[3*N_0 + 5: 4*N_0 + 5]
#     eM = init[4*N_0 + 5: 5*N_0 + 5]

#     dSdt = (b_S - (d_S)*S - (I >= I_0)*b_I*I*S*(1-kappa*H**l_H/(K_IH**l_H + H**l_H)) - S*(d_IH*H**l_H/(K_IH**l_H + H**l_H) + d_IE*(Eout_pop)/(K_SE + S + Eout_pop)))*(S >= 0.0)
#     dIdt = ((I >= I_0)*b_I*I*S*(1-kappa*H**l_H/(K_IH**l_H + H**l_H)) - d_IH*I*H**l_H/(K_IH**l_H + H**l_H) - d_IE*I*(Eout_pop)/(K_IE + I + Eout_pop) - d_I*I)*(I >= 0.0)
#     dHdt = (b_H*(cell_lysis_rate) - d_H*H)*(H >= 0.0)
#     dAoutdt = b_Ain*Aout*H**l_H/(K_HE**l_H + H**l_H)*dt*(Aout >= 0.0)
#     dAindt = (b_Ain*Aout*H**l_H/(K_HE**l_H + H**l_H) - d_A*Ain - d_IE*Ain*(cMa_pop)/(K_IE/delta + Ain + cMa_pop))*(Ain >= 0.0)
#     dNdt = 
#     dNadt =
#     dEdt =
#     dcMdt =
...
param_names_for_df = ['S_0', 'I_0', 'b_I', 'd_S', 'd_I', 'd_IE', 'd_IH', 'K_IE',
                      'K_IH', 'Aout_0', 'b_Ain', 'b_H', 'd_H', 'K_HE',
                      'N_0', 'max_Na', 'b_myc', 'd_myc', 'myc_thresh',
