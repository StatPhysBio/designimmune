...
def agent_stoch_sim(I_0 = I_0, b_I = b_I, t1 = t1, d_IE = d_IE, T_I = T_I,
    ...
    for i in np.arange(1, steps + 1):
        ...
        N_to_cM = np.random.binomial(N_act.astype(int), 
                                     np.nan_to_num((1-psi_N_I)*(1-p_t) + psi_N_I*p_t), N0)

        # central memory cells divide and differentiate
        cM_div = np.random.binomial(cM_m[i-1,:].astype(int), 
                                    np.nan_to_num(dt*b_cM(t1,I[i-1], E_pop, Treg0, max_val = div_rates[2])), N0)
        cM_to_E = np.random.binomial(2*cM_div.astype(int), 
                                     np.nan_to_num((1-psi_N_I)*(1-p_t) + psi_N_I*p_t), N0)

        # effector cells divide and differentiate, or die, or both
        E_div = np.random.binomial(-np.minimum(-E_m[i-1,:].astype(int),0), 
                                   np.nan_to_num(dt*b_E(t1,I[i-1], E_pop, Treg0, max_val = div_rates[1], b_c1_pop = np.mean(div_rates[3]))), N0)
        E_to_eM = np.random.binomial(E_div.astype(int), 
                                     np.nan_to_num(alpha*((1-psi_E_I)*(1-p_t) + psi_E_I*p_t) + (1-alpha)*((1 - psi_E_c)*(1-hl_t) + psi_E_c*hl_t)), N0)
        E_death = np.random.binomial(-np.minimum(-E_m[i-1,:].astype(int),0), 
                                     np.nan_to_num(dt*d_E(t1,I[i-1], E_pop, Treg0, b_E_pop = div_rates[2], b_c1_pop = np.mean(div_rates[3]))), N0)

        E_to_M_death = np.random.binomial(E_to_eM.astype(int), 
                                          np.nan_to_num(dt*d_E(t1,I[i-1], E_pop, Treg0, b_E_pop = div_rates[2], b_c1_pop = np.mean(div_rates[3]))), N0)
