...
NE_psis = [psi_max/2, psi_max/2, 0.0, psi_max/2] # regulatory weights: psi_M_I, psi_M_H, psi_M_P
EM_psis = [-psi_max/2, -psi_max/2, 0.0, -psi_max/2]
...
def lin_stoch_sim(S_0 = S_0, I_0 = I_0, b_I = b_I, d_S = d_S, d_I = d_I, d_IE = d_IE, K_IE = K_IE, K_SE = K_SE,
    ...
    for i in np.arange(1, int(steps) + 1):
        ...
        Ma_m[i] += Ma_m[i-1] + div_Ma + (1 - Na_div_flag)*(Na_m[i-1] - diff_NaE) + diff_EM
        ...
        r_NaE[i] += (r_NaE[i-1] + 2*dt*f_XtoY(sig_1 = p_tcr*cells_sensed, sig_2 = p_cyt*H[i], sig_3 = p_cyt*P[i], psi_1 = psi_NE_I, psi_2 = psi_NE_H, psi_3 = psi_NE_P,F_0 = F0_NE, K_1 = (K_IE*(vir_model != "autoimmune") + K_SE*(vir_model == "autoimmune")), K_2 = K_EH, K_3 = K_EH, reg_model = reg_model)/(2/np.sqrt(np.pi)*(char_times[6]))**2)*(N_m[i]*bound_N > 0)*(mycN >= myc_thresh)
        ...
        for l in np.where((N_m[i] == 0)*(E_m[i] + b_Ma_div > 0))[0]:
            ...
            # T_Ecyt[0] += np.sum(diff_NaE[l] + div_Ma[l])
            T_Ecyt[l] += (diff_NaE[l] + div_Ma[l])*[0.0] + diff_EM[l]*[T_E[l].item()]

        # T_Ecyt += np.sum(diff_NaE + div_Ma)*[0.0] + [diff_EM*T_E.tolist()]
        ...
        bias_t[i] += np.array([np.mean((r_Nact[i])) if N_0 > 0.0 else 0.0,
                               np.mean((r_NaE[i])[N_m[i]*bound_N > 0]) if np.sum(N_m[i]*bound_N) > 0.0 else 0.0, 
...
reg_or = [Nact_reg[0:2]] + [NE_reg[0:2]] + [EM_reg[0:2]] + [Ediv_reg[0:2]]
reg_and = [Nact_reg[2]] + [NE_reg[2]] + [EM_reg[2]] + [Ediv_reg[2]]
reg_and_label = [param_names[-14]] + [param_names[-10]] + [param_names[-6]] + [param_names[-2]]
reg_bl = [Nact_reg[3]] + [NE_reg[3]] + [EM_reg[3]] + [Ediv_reg[3]]
